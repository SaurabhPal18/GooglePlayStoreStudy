...
data=pd.read_csv("data.csv")#DataFrames is two-dimensional(2-D) data structure defined in pandas which consists of rows and columns.
data["Is"]=list(data.Installs)
data["Inst"]=list(data.Installs)
...
def point_10():
    screen_10=Tk()
    adjustWindow(screen_10)
    
    
    Categorys=data.Category.unique()
    Label(screen_10,text="Category                                      month in 2014 with highest install      month in 2015 with highest install       month in 2016 with highest install    month in 2017 with highest install    month in 2018 with highest install   ",font=("Open Sans",10,'bold'),fg="Black").place(x=0,y=0)
    #print(Categorys)
    pp=20
    pp1=20
    for category in Categorys:
        ...
        cat=data.loc[data['Category']==category]#loc function is used for returning rows
        cat=cat.sort_values('Last Updated')
        installs = [ float(i.replace('+','').replace(',', '')) if '+' in i or ',' in i else float(0) for i in cat["Inst"] ]
        
        Label(screen_10,text=category,font=("Open Sans",10,'bold'),fg="Black").place(x=0,y=pp)
        pp=pp+20
    #    print(category)
    #    print(installs)
        dates=list(cat['Last Updated'])
    ...
    #    print(list(dates))
        ...
        for year in ['2014','2015','2016','2017','2018']:
            ...
            for month in months:
                ...
                for i in range(len(dates)):
                    j=dates[i].split()
                    if(j[0]==month and j[2]==year):
                        sum=sum+installs[i]
                dic.update({month:sum})
            list_sum.append(list(dic.values()))
    #        print(dic)
#        x=['jan','feb','mar','apr','may','june','july','aug','sep','oct','nov','dec']
    ...
    #    print(a)
        m=max(a)
        if(m==0):
            Label(screen_10,text="N.A",font=("Open Sans",10,'bold'),fg="Black").place(x=250,y=pp1)
        else:
            ...
            for i in range(len(a)):
                if a[i]==m:
                    break
            Label(screen_10,text=months[i],font=("Open Sans",10,'bold'),fg="Black").place(x=250,y=pp1)
        
    
        n=max(b)
        if(n==0):
            Label(screen_10,text="N.A",font=("Open Sans",10,'bold'),fg="Black").place(x=480,y=pp1)
            ...
            for i in range(len(b)):
                if b[i]==n:
                    break
            Label(screen_10,text=months[i],font=("Open Sans",10,'bold'),fg="Black").place(x=480,y=pp1)
        o=max(c)
        if(o==0):
            Label(screen_10,text="N.A",font=("Open Sans",10,'bold'),fg="Black").place(x=710,y=pp1)
        else:
            ...
            Label(screen_10,text=months[j],font=("Open Sans",10,'bold'),fg="Black").place(x=710,y=pp1)
        p=max(d)
        if(p==0):
            Label(screen_10,text="N.A",font=("Open Sans",10,'bold'),fg="Black").place(x=940,y=pp1)
        else:
            for i in range(len(d)):
                if d[i]==p:
                    break
            Label(screen_10,text=months[i],font=("Open Sans",10,'bold'),fg="Black").place(x=940,y=pp1)
        q=max(e)
        if(q==0):
            Label(screen_10,text="N.A",font=("Open Sans",10,'bold'),fg="Black").place(x=940+230,y=pp1)
        else:
            for i in range(len(e)):
                if e[i]==q:
                    break
            Label(screen_10,text=months[i],font=("Open Sans",10,'bold'),fg="Black").place(x=940+230,y=pp1)
        pp1=pp1+20
    
    sumt=0
    smto=0
    teen=data.loc[data['Content Rating']=='Teen']
    teenI=list(teen["Installs"])

    for i in range(len(teenI)):
        sumt=sumt+teenI[i]
    
    mat=data.loc[data['Content Rating']=='Mature 17+']
    teenIn=list(mat["Installs"])
    for i in range(len(teenIn)):
        smto=smto+teenIn[i]

    ratio=sumt/smto
    ans="The ratio of downloads for the app that qualifies as teen versus mature17+ is :: "+str(ratio)
    Label(screen_10,text=ans,font=("Open Sans",10,'bold'),fg="Black").place(x=0,y=680)
    Label(screen_10,text="Here N.A means Not Appliable means in that year the particular category of app wasn't installed",font=("Open Sans",10,'bold'),fg="Red").place(x=740,y=680)    
...
#        app_sentiment3.update({i:c})
    ...
    listbox=Listbox(screen_12,height=18,width=50)
...
def point_14():
    def po():
        Label(screen_14,text="                                                                                                                                                                                                                  " ,bg="white").place(x=0,y=200)
        a=variable.get()
    #    Label(screen_14,text=a,bg="white",fg="red").place(x=0,y=200)
        apprev=data1.loc[data1['App']==a]
        appre=apprev.loc[apprev['Sentiment']=='Positive']
        appr=apprev.loc[apprev['Sentiment']=='Negative']
        appr1=apprev.loc[apprev['Sentiment']=='Neutral']
        listbox=Listbox(screen_14,height=7,width=250)
        listbox1=Listbox(screen_14,height=7,width=250)
        listbox2=Listbox(screen_14,height=7,width=250)
    #    scrool=Scrollbar(screen_14,orient="vertical")
    #    scrool.pack(side=tk.RIGHT, fill=tk.Y)
        listbox.place(x=0,y=90)
        listbox1.place(x=0,y=240)
        listbox2.place(x=0,y=390)
    #    scrool.config(command=listbox.yview)
    #    listbox.config(yscrollcommand=scrool.set)
        listbox.insert(tk.END,*appre['Translated_Review'])
        listbox1.insert(tk.END,*appr['Translated_Review'])
        listbox2.insert(tk.END,*appr1['Translated_Review'])
    ...
    screen_14=Tk()
    adjustWindow(screen_14)    
    app_array=data1.App.unique()
    no_emoji=[]
    emoji_pattern = re.compile("["u"\U0001F600-\U0001F64F"u"\U0001F300-\U0001F5FF"u"\U0001F680-\U0001F6FF"u"\U0001F1E0-\U0001F1FF""]+", flags=re.UNICODE)
    for i in app_array:
    #    emoji_pattern.sub(r'',i)
    ...
    variable=StringVar(screen_14)
    variable.set('Select app')
    #variable.set(no_emoji[0])
    w=OptionMenu(screen_14,variable,*no_emoji)
    w.place(x=0,y=0)
    Button(screen_14,text="ok",bg="#990011",width=20,height=1,font=("Open Sans",10,'bold'),fg="white",command=po).place(x=0,y=30)
    Label(screen_14,text="Positive Review",bg="cyan",fg="red").place(x=0,y=60)
    Label(screen_14,text="Negative Review",bg="cyan",fg="red").place(x=0,y=220)
    Label(screen_14,text="Neutral",bg="cyan",fg="red").place(x=0,y=370)
def point_15():
    global screen_15
    screen_15=Tk()
...
def point_17():
    global screen_17
    screen_17=Tk()
    screen_17.title(" month(s) of the year , is the best indicator to the avarage downloads that an app will generate over the entire year")
    adjustwindow(screen_17)
...
#    plt.figure(figsize=(14,8))
    fig=Figure(figsize=(8,5))
    a=fig.add_subplot(111)
    a.scatter(size1,Andriod_version['Installs'],c='yellow')
    a.plot(size1,predictions,c='red',linewidth=2)
    a.set_xlabel("Size in Mb")
    a.set_ylabel("Installs")
    canvas = FigureCanvasTkAgg(fig,master=screen_17)
    canvas.get_tk_widget().pack()
    canvas.draw()
    TXt="There is a very slight positive relatiion between Size and Installs"
    Label(screen_17,text=TXt,bg="#663399",font=("Open Sans",13,'bold'),fg="white").pack()
...
def reg2():
    ...
    def submit2():
        ...
        connection = pymysql.connect(host="localhost", user="root", passwd="", database="data") # database connection 
...
def register():
    ...
    Label(screen1, text="Review Given:", font=("Open Sans", 11, 'bold'), fg='white',bg='#174873', anchor=W).place(x=0, y=210)
    Entry(screen1, textvar=rev).place(x=150, y=210)
    Label(screen1, text="Rating out of 5", fg="green",font=("calibri", 11), width='30', anchor=W, bg='white').place(x=300, y=310)
    ...
    Label(screen1, text="DD/MM/YY", fg="green",font=("calibri", 11), width='30', anchor=W, bg='white').place(x=300, y=560)
...
Button(screen,text="App Reviews",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),fg="white",command=point_14).place(x=30,y=158+30+32)
Button(screen,text="category got Most & Least Download(2016,2017,2018)",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),fg="white",command=point_6).place(x=200,y=190+30+30+20)
Button(screen,text="Apps with Maximum Compatibility",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),fg="white",command=point_7).place(x=200,y=222+30+30+20)
#Button(mainscreen,text="Feature 8",bg="#e79700",width=60,height=1,font=("Open Sans",13,'bold'),fg="white").place(x=30,y=30)
Button(screen,text="Apps 1,00,000+ downloads and rated 4.1 or above",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),fg="white",command=point_9).place(x=200,y=254+30+30+20)
Button(screen,text="quaterly download of each year",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),command=point_11,fg="white").place(x=200,y=286+30+30+20)
Button(screen,text="Highest Install of each category in each month of each year",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),fg="white",command=point_10).place(x=200,y=286+62+30+20)
...
Button(screen,text="Apps generated most Positive, Negative & equal Sentiments",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),fg="white",command=point_12).place(x=300,y=360+30+30+25)
Button(screen,text="Relation B/W Sentiment-polarity & Sentiment-subjectivity",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),fg="white",command=point_13).place(x=300,y=392+30+30+25)
Button(screen,text="Month having highest average download by apps",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),fg="white",command=point_16).place(x=300,y=424+30+30+25)
Button(screen,text="Relation between size and Installs",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),fg="white",command=point_17).place(x=300,y=456+30+30+25)
Button(screen,text="Next",bg="#990011",width=20,height=1,font=("Open Sans",14,'bold'),fg="yellow",command=main_screenok).place(x=825,y=600+25)       
Button(screen,text="10 best food App",bg="#990011",width=60,height=1,font=("Open Sans",14,'bold'),fg="white",command=point_15).place(x=300,y=488+30+30+25)       
